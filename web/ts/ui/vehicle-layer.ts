...
export class VehicleLayer {
    ...
    #onVehicleGone: (id: string) => void;

    constructor(canvas: HTMLCanvasElement, { onVehicleGone }: { onVehicleGone?: (id: string) => void } = {}) {
        this.#ctx = canvas.getContext('2d')!;
        this.#onVehicleGone = onVehicleGone ?? (() => {});
        canvas.width  = W;
        canvas.height = H;
    }

    draw(vehicles: Vehicle[], timestamp: number): void {
        const ctx = this.#ctx;
    ...
    #drawWaiting(ctx: CanvasRenderingContext2D, v: Vehicle): void {
        const pos = queuePos(v.startRoad, v.lane ?? 0, v.queueIndex);
        this.#drawVehicle(ctx, pos.x, pos.y, v, 1);
    }

    #animateLeaving(ctx: CanvasRenderingContext2D, v: Vehicle, timestamp: number): void {
        ...
        if (raw >= 1) this.#onVehicleGone(v.id);
    }

    #drawVehicle(ctx: CanvasRenderingContext2D, x: number, y: number, v: Vehicle, alpha: number): void {
        if (alpha <= 0) return;
        ctx.save();
